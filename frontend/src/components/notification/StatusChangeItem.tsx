...
export function StatusChangeItem({ notification, onMarkRead, onClose }: StatusChangeItemProps) {
    ...
    <div className="flex gap-3">
      ...
      <div className="flex-1 flex flex-col gap-1">
        ...
        <div className="flex items-start justify-between gap-2">
          <span className={`text-base md:text-sm font-manrope ${!notification.isRead ? "font-semibold text-[#020617]" : "font-medium text-[#020617]"}`}>
        ...
        {/* Description */}
        <p className="text-sm text-[#64748B] font-manrope">
        ...
        {(notification.orderDate ||
          notification.orderItemsCount ||
          notification.orderTotal) && (
          <div className="flex items-center gap-2 text-xs text-[#64748B] font-manrope mt-0.5">
            {notification.orderDate && (
  ...
  const baseClassName = `w-full text-left px-4 md:px-6 py-5 md:py-4 hover:bg-[#F8FAFC] transition-colors border-b border-[#E2E8F0] last:border-b-0 ${!notification.isRead ? "bg-[#F8FAFC]" : ""}`;
