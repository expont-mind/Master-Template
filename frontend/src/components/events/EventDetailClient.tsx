...
function getEventStatus(
  ...
  if (startDate) {
    const start = parseAsUTC(startDate).toLocaleDateString("en-CA", {
      timeZone: "Asia/Ulaanbaatar",
    });
    if (todayUB < start) return "upcoming";
  }

  if (endDate) {
    const end = parseAsUTC(endDate).toLocaleDateString("en-CA", {
      timeZone: "Asia/Ulaanbaatar",
    });
    if (todayUB > end) return "ended";
  }
          ...
          {event.image_url && (
            <div className="relative w-full h-[280px] md:h-[448px] rounded-sm overflow-hidden">
              <Image
                ...
                alt={event.title}
                fill
                className="object-contain object-center bg-[#F1F5F9]"
                sizes="1064px"
                quality={75}
                priority
              />
