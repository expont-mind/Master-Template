...
function SidebarArticleCard({ article }: { article: Article }) {
  ...
  return (
    <Link href={`/articles/${article.slug}`} className="flex flex-col gap-2">
      <div className="relative w-full aspect-[3/2] bg-[#F1F5F9] rounded-sm overflow-hidden">
        {article.is_featured && (
          <div className="absolute top-2 left-2 z-10">
            <span className="px-2 py-0.5 bg-[#020617] rounded text-white font-medium text-xs font-manrope">
              Онцлох
            </span>
          </div>
        )}
      ...
      <div className="flex flex-col gap-1">
        <h3 className="text-[#020617] font-medium text-sm font-manrope leading-5 line-clamp-2">
          {article.title}
        </h3>
        {displayDate && (
        ...
        <div className="flex flex-col lg:flex-row gap-8 lg:gap-10 mt-4">
          ...
          <div className="flex-1 flex gap-4">
            ...
            <div className="hidden md:flex flex-col gap-4 pt-2">
              <button className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-[#F1F5F9] transition-colors duration-200">
                <LinkIcon />
              </button>
              <button className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-[#F1F5F9] transition-colors duration-200">
                <MailIcon />
            ...
            <div className="flex-1 flex flex-col gap-6">
              ...
              {displayDate && (
                <div className="flex flex-col gap-1.5">
                  <span className="text-[#64748B] font-medium text-base font-manrope">
                    Нийтэлсэн огноо
                  </span>
              ...
              {article.content && (
                <div className="flex flex-col gap-1.5">
                  ...
                  <div className="text-[#020617] font-medium text-base font-manrope whitespace-pre-line">
                    {article.content}
                  </div>
                </div>
              )}
