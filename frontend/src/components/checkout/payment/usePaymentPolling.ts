...
export function usePaymentPolling({
  ...
  // "transfer" is a manual flow — no polling.
  ...
  // Realtime subscription for order payment_status changes (admin updates)
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          const newPaymentStatus = (payload.new as { payment_status?: string })
            ?.payment_status;
