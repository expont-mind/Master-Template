...
export function useDeliveryEdit(id: string) {
  ...
  const saveMutation = useMutation({
    mutationFn: (data: Record<string, unknown>) =>
      isNew
        ? adminApi.insert("delivery_zones", data)
        : adminApi.update("delivery_zones", id, data),
    onSuccess: () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage = translateServerError(err instanceof Error ? err.message : "", "Хүргэлтийн бүс хадгалахад алдаа гарлаа.");
