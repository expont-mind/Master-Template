...
export function useAdsEdit(id: string) {
  ...
  const saveMutation = useMutation({
    mutationFn: (data: Record<string, unknown>) =>
      isNew
        ? adminApi.insert<Ad>("ads", data)
        : adminApi.update<Ad>("ads", id, data),
    onSuccess: (result) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError(translateServerError(error instanceof Error ? error.message : "", "Зар хадгалахад алдаа гарлаа."));
    } finally {
      setIsSaving(false);
    }
  };
