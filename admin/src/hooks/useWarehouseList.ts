...
export function useWarehouseList() {
  ...
  useResetPage(() => setPage(1), [debouncedSearch, statusFilter, typeFilter]);
  ...
  const { data, isLoading } = useQuery({
    queryKey: queryKeys.warehouses.lists({ page, search: debouncedSearch, status: statusFilter, type: typeFilter }),
    queryFn: () =>
      adminApi.getAllPaginated<Warehouse>("warehouses", {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => adminApi.delete("warehouses", id),
    onSuccess: () => {
  ...
  return {
    ...
    setDeleteTarget,
    handleDelete,
    refetch: () => queryClient.invalidateQueries({ queryKey: queryKeys.warehouses.all }),
    page,
    setPage,
    totalCount,
    totalPages,
