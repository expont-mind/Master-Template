...
export function ReturningVsNewChart({ data, isLoading }: ReturningVsNewChartProps) {
  ...
  return (
    <ChartCard title="Шинэ vs Буцсан худалдан авагч" isLoading={isLoading} isEmpty={!data} height={280}>
      <ResponsiveContainer width="100%" height={280}>
        <BarChart data={chartData}>
          <CartesianGrid strokeDasharray="3 3" className="stroke-muted" />
          <XAxis dataKey="name" {...CHART_AXIS_STYLE} tick={{ ...CHART_AXIS_STYLE.tick, fontSize: 11 }} />
          <YAxis {...CHART_AXIS_STYLE} />
          <Tooltip {...CHART_TOOLTIP_STYLE} formatter={(value: number | undefined) => [value ?? 0, "Худалдан авагч"]} />
