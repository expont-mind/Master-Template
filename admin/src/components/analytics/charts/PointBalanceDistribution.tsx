...
export function PointBalanceDistribution({ data, isLoading }: PointBalanceDistributionProps) {
  return (
    <ChartCard title="Оноо тархалт" isLoading={isLoading} isEmpty={data.every((d) => d.count === 0)} height={280}>
      <ResponsiveContainer width="100%" height={280}>
        <BarChart data={data}>
          <CartesianGrid strokeDasharray="3 3" className="stroke-muted" />
          <XAxis dataKey="label" {...CHART_AXIS_STYLE} tick={{ ...CHART_AXIS_STYLE.tick, fontSize: 11 }} />
          <YAxis {...CHART_AXIS_STYLE} />
          <Tooltip {...CHART_TOOLTIP_STYLE} formatter={(value: number | undefined) => [value ?? 0, "Хэрэглэгч"]} />
