...
export function StockDistribution({ data, isLoading }: StockDistributionProps) {
  return (
    <ChartCard title="Нөөцийн тархалт" isLoading={isLoading} isEmpty={data.every((d) => d.count === 0)}>
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={data}>
          <CartesianGrid strokeDasharray="3 3" className="stroke-muted" />
          <XAxis dataKey="label" {...CHART_AXIS_STYLE} />
          <YAxis {...CHART_AXIS_STYLE} />
          <Tooltip {...CHART_TOOLTIP_STYLE} formatter={(value: number | undefined) => [value ?? 0, "Бүтээгдэхүүн"]} />
