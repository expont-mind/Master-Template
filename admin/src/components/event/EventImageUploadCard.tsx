...
interface EventImageUploadCardProps {
  imageUrl: string;
  setImageUrl: (url: string) => void;
  disabled?: boolean;
}

export function EventImageUploadCard({
  imageUrl,
  setImageUrl,
  disabled,
}: EventImageUploadCardProps) {
  ...
  const processFile = async (file: File) => {
    try {
      ...
      log.error("event_image_upload_failed", err);
    } finally {
  ...
  return (
    <Card>
      <CardContent className="p-6 space-y-4">
        <h3 className="text-lg font-semibold">Эвентийн зураг</h3>
