    ...
    <Card>
      ...
      <CardContent className="space-y-2 text-sm">
        <div className="flex justify-between">
          ...
          <span>{new Date(event.created_at).toLocaleDateString("mn-MN", { timeZone: "Asia/Ulaanbaatar" })}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-muted-foreground">Шинэчилсэн:</span>
          <span>{new Date(event.updated_at).toLocaleDateString("mn-MN", { timeZone: "Asia/Ulaanbaatar" })}</span>
        </div>
      </CardContent>
    </Card>
  );
