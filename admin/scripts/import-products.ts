...
async function processBatch(products: ProductJSON[], batchNumber: number, totalBatches: number): Promise<ImportResult[]> {
...
async function main() {
  ...
  try {
    ...
    for (let i = 0; i < newProducts.length; i += BATCH_SIZE) {
      ...
      console.log(`Progress: ${i + batch.length}/${newProducts.length} (${progress}%) - ${successCount} successful`);
    }
