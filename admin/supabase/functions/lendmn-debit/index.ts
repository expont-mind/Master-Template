...
async function lendmnFetch<T>(
  ...
  if (!res.ok) {
    throw new Error(
      `LendMN API error ${res.status}: ${JSON.stringify(json)}`,
    );
  }
  ...
  try {
    ...
    switch (action) {
      case "create-invoice": {
        ...
        const invoice = await lendmnFetch<LendMNInvoiceResponse>(
        ...
        console.log("[lendmn-debit] LendMN response keys:", Object.keys(invoice));
        ...
        if (insertError) {
          console.error("[lendmn-debit] DB insert error:", insertError.message);
          return jsonResponse(
      ...
      case "get-invoice": {
        ...
        const invoice = await lendmnFetch(
    ...
    console.error("[lendmn-debit] Error:", err);
    return jsonResponse(
      {
