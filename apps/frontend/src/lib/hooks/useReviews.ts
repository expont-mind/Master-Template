...
// All review queries are inert when the reviews feature is disabled —
// consumers (cards, detail page) need no flag awareness of their own.
const REVIEWS_ENABLED = SITE.features.reviews;
...
export function useReviews(productId: string, rating?: number | null) {
  return useInfiniteQuery({
    ...
    enabled: REVIEWS_ENABLED && !!productId,
  });
}
...
export function useCanReview(productId: string, userId: string | null) {
  return useQuery({
    ...
    staleTime: CACHE_TTL.PRODUCT_DETAIL * 1000,
    enabled: REVIEWS_ENABLED && !!productId && !!userId,
  });
}
 ...
 */
export function useCanReviewAny(productIds: string[], userId: string | null) {
  return useQuery({
    queryKey: [...reviewKeys.all, "canReviewAny", ...productIds.toSorted(), userId ?? ""],
    queryFn: () => checkCanReviewAny(productIds, userId!),
    staleTime: CACHE_TTL.PRODUCT_DETAIL * 1000,
    enabled: REVIEWS_ENABLED && productIds.length > 0 && !!userId,
  });
}
