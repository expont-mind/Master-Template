...
function _assertAuthEnv(): void {
  if (STOREPAY_AUTH_URL && STOREPAY_USERNAME && STOREPAY_PASSWORD) return;
  const missing = [
    ...
    .filter(Boolean)
    .join(", ");
  throw new Error(`StorePay env vars missing: ${missing}`);
}

function _buildAuthUrl(): string {
  return `${STOREPAY_AUTH_URL}?grant_type=password&username=${encodeURIComponent(STOREPAY_USERNAME)}&password=${encodeURIComponent(STOREPAY_PASSWORD)}`;
}

function _buildBasicAuthHeader(): string {
  const basicAuth = Buffer.from(`${STOREPAY_APP_USERNAME}:${STOREPAY_APP_PASSWORD}`).toString(
    "base64",
  );
  return `Basic ${basicAuth}`;
}

async function _requestNewToken(): Promise<{
  token: string;
  expiresIn: number;
}> {
  const res = await fetch(_buildAuthUrl(), {
    method: "POST",
    headers: {
...
async function getAccessToken(): Promise<string> {
  ...
  const { token, expiresIn } = await _requestNewToken();
  accessToken = token;
...
async function storepayFetch<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
  ...
  if (!res.ok) {
    throw new Error(`StorePay API error ${res.status}: ${JSON.stringify(json)}`);
  }
...
export async function createStorePayInvoice(
  ...
  if (raw.status !== "Success" || raw.value == null) {
    const errorMsg = raw.msgList?.[0]?.code ?? "StorePay нэхэмжлэл үүсгэхэд алдаа гарлаа";
...
export async function getStorePayInvoiceStatus(
  ...
  const data = await storepayFetch<StorePayCheckResponse>(`/merchant/loan/check/${loanId}`);

  const isPaid =
    data.status === "Success" && data.value === true && data.data?.isConfirmed === true;
