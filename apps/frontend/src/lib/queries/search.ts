...
/**
 * Fetch product-level images in URL-length-safe parallel batches and
 * accumulate them into `imageMap` keyed by product_id.
 */
async function fetchProductLevelImages(
...
/**
 * For any productIds that have no product-level image, find the default
 * (or first) active variant and use its image instead.
 */
async function fetchFallbackVariantImages(
  supabase: SupabaseClient,
  productIdsMissingImage: string[],
  imageMap: Map<string, string[]>,
): Promise<void> {
  if (productIdsMissingImage.length === 0) return;

  const { data: variants } = await supabase
    .from("product_variants")
    .select("id, product_id, is_default")
    .in("product_id", productIdsMissingImage)
    .eq("status", "active")
...
async function attachProductImages<T extends { id: string; images: string[] }>(
  ...
  await fetchProductLevelImages(supabase, productIds, imageMap);
  const missing = productIds.filter((id) => !imageMap.has(id));
  await fetchFallbackVariantImages(supabase, missing, imageMap);
...
type SearchProductsRow = ProductListItem & {
  total_count: number;
  fuzzy_fallback?: boolean;
};

interface SearchRpcArgs {
...
function buildSearchRpcArgs(filters: ProductFilters, page: number, limit: number): SearchRpcArgs {
  return {
...
// Explicit pick strips `total_count` and `fuzzy_fallback` from rows.
function mapSearchRowToProduct(row: SearchProductsRow): ProductListItem {
  return {
    id: row.id,
    name: row.name,
    slug: row.slug,
...
export async function searchProducts(filters: ProductFilters): Promise<SearchResult> {
  ...
  const { data, error } = await supabase.rpc(
    "search_products",
    buildSearchRpcArgs(filters, page, limit),
  );
  ...
  const products = rows.map(mapSearchRowToProduct);
...
export async function logSearch(query: string, resultCount: number): Promise<void> {
  ...
  let sessionId =
    typeof window !== "undefined" ? sessionStorage.getItem("monpang:session-id") : null;
...
export async function getTrendingSearches(limit = 5): Promise<TrendingSearch[]> {
  const supabase = createClient();
