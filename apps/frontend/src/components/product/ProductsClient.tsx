...
const SIDEBAR_FALLBACKS = ["s1", "s2", "s3", "s4", "s5", "s6", "s7", "s8"] as const;
const PRODUCT_FALLBACKS = ["p1", "p2", "p3", "p4", "p5", "p6", "p7", "p8", "p9"] as const;

function findInTree(cats: CategoryWithChildren[], slug: string): CategoryWithChildren | null {
...
function findPath(cats: CategoryWithChildren[], slug: string): CategoryWithChildren[] | null {
  for (const cat of cats) {
    if (cat.slug === slug) return [cat];
...
function ProductsPageContent() {
  ...
  const breadcrumbPath = selectedSlug ? findPath(categories, selectedSlug) : null;

  const parentWithChildren = selectedSlug ? findParentWithChildren(categories, selectedSlug) : null;
    ...
    <div className="w-full bg-white flex justify-center min-h-screen">
      <div className="flex flex-col max-w-[1064px] w-full pb-20">
        <div className="py-2.5 md:pt-7 md:pb-2 flex items-center gap-4 lg:gap-16 px-4 xl:px-0">
          <h1 className="lg:max-w-[280px] lg:w-full px-1.5 text-text-primary font-bold text-xl md:text-[26px] leading-9 font-manrope tracking-[-0.26px] shrink-0 hidden md:block">
            Категори
          </h1>
        ...
        <div className="flex gap-4 lg:gap-16">
          ...
          <div className="flex flex-col flex-1">
            {/* Subcategory tabs */}
            {parentWithChildren && (
              <SubcategoryTabs parentCategory={parentWithChildren} selectedSlug={selectedSlug} />
            )}
