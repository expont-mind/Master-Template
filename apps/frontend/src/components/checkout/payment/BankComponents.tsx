...
export function BankField({
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <p className="text-text-primary font-normal text-sm font-manrope leading-6">{label}</p>
      <div className="flex items-center gap-2 h-12 pl-3 pr-0.5 bg-border-light rounded-sm">
        <p
          className={`text-text-primary ${semibold ? "font-semibold" : "font-normal"} text-sm font-manrope w-full`}
        >
          {value}
        </p>
        {copyable && (
...
export function getBankDescription(name: string): string {
  ...
  if (lowerName.includes("mbank") || lowerName.includes("m bank")) return "Мобайл апп ашиглах";
    ...
    >
      {bank.logo ? (
        ...
        <div className="w-12 h-12 rounded-lg bg-border-light flex items-center justify-center shrink-0">
          <span className="text-text-secondary text-sm font-bold">{bank.name.charAt(0)}</span>
        </div>
      )}
      <div className="flex flex-col min-w-0">
        <PrimarySm>{bank.name}</PrimarySm>
        {description && (
          <p className="text-text-secondary font-normal text-xs font-manrope hidden md:block">
            {description}
          </p>
        )}
      </div>
