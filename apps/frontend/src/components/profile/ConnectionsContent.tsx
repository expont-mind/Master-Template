...
import { GoogleIcon, FacebookIcon, X, Slash } from "@/components/svg";
...
interface ConnectionRowProps {
  conn: ConnectionItem;
  onDisconnect: (conn: ConnectionItem) => void;
  onConnect: (provider: Provider) => void;
}

function ConnectionRow({ conn, onDisconnect, onConnect }: ConnectionRowProps) {
  return (
    <div className="flex items-center gap-4 px-0.5">
      <div className="shrink-0">{conn.icon}</div>
      <div
        className={`flex-1 flex flex-col gap-1 items-start py-4 ${
          conn.hasBorder ? "border-b border-border" : ""
        }`}
      >
        <p
          className={`font-manrope text-sm leading-5 ${
            conn.connected ? "text-text-secondary font-medium" : "text-text-primary font-semibold"
          }`}
        >
          {conn.name}
        </p>
        {conn.connected ? (
          <div className="flex items-center gap-1 bg-teal-50 rounded-[4px] px-2 py-0.5">
            <p className="text-teal-500 font-medium text-sm font-manrope leading-5">
              Холболт хийгдсэн
            </p>
            <button
              type="button"
          ...
          <button
            onClick={() => onConnect(conn.id as Provider)}
            className="border border-brand-primary/40 text-brand-primary font-medium text-sm font-manrope leading-5 px-2 py-0.5 rounded-[4px] cursor-pointer hover:bg-status-error-bg transition-colors"
          >
            Холбох
          </button>
        )}
  ...
  const handleConfirmDisconnect = async () => {
    ...
    try {
      ...
      if (!result.success) {
        ...
        setIdentities((prev) => prev.filter((i) => i.id !== disconnectModal.identity?.id));
      }
    } catch (err) {
    ...
    <div className="flex flex-col gap-4 md:gap-2">
      <div className="flex flex-col">
        <div className="flex items-center gap-1.5 px-px md:px-0.5">
          ...
          <Slash />
          <p className="text-slate-950 text-sm font-normal font-manrope">Холболт</p>
        </div>
