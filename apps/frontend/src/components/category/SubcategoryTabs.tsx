...
export function SubcategoryTabs({ parentCategory, selectedSlug }: SubcategoryTabsProps) {
  ...
  return (
    <div className="pb-0 md:pb-4">
      <div className="bg-surface py-2 md:py-1.5 rounded-sm grid grid-cols-3 gap-0.5 md:gap-x-4 md:gap-y-[2px]">
        <button
        ...
        {parentCategory.children.map((sub) => (
          <button
            key={sub.id}
            className={`px-4 py-2.5 text-left text-sm font-manrope cursor-pointer hover:text-text-primary whitespace-nowrap truncate transition-all ${
