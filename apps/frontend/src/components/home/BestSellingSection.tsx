...
export const BestSellingSection = ({ initialProducts }: BestSellingSectionProps) => {
        ...
        <div className="flex flex-col gap-8 pt-4">
          <div className="flex gap-4 overflow-x-auto scrollbar-hide">
            {firstRow.map((product, i) => (
              <ProductCard variant="horizontal" key={product.id} product={product} rank={i + 1} />
            ))}
          </div>
          {secondRow.length > 0 && (
            <div className="flex gap-4 overflow-x-auto scrollbar-hide">
              {secondRow.map((product, i) => (
                <ProductCard variant="horizontal" key={product.id} product={product} rank={i + 6} />
              ))}
            </div>
          )}
        </div>
