...
const bannerConfig = {
  product: {
    title: null,
    description: "Таны захиалсан бараа Солонгос улсаас Монгол улсруу 3-5 хоногт хүргэгдэнэ.",
    bgColor: "bg-blue-400",
...
export function StaticBanner({ variant = "product", className = "" }: StaticBannerProps) {
  const config = bannerConfig[variant];

  return (
    <div className={` py-10 md:py-28 bg-white md:bg-surface px-4 md:px-0 ${className}`}>
      <div className="w-full max-w-[720px] mx-auto">
        ...
        >
          ...
          >
            {config.title && (
              <p className="text-lg font-bold leading-7 hidden md:block">{config.title}</p>
            )}
            <p className="text-xs md:text-base font-medium">{config.description}</p>
          </div>
