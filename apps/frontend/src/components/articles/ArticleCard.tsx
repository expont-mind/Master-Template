...
export function ArticleCard({ article }: ArticleCardProps) {
  ...
  return (
    ...
    >
      <div className="relative w-full aspect-[3/2] bg-surface rounded-lg overflow-hidden">
        {article.is_featured && (
          <div className="absolute top-3 left-3 z-10">
            <span className="px-2.5 py-1 bg-white rounded-full text-text-primary font-medium text-xs font-manrope shadow-sm">
              Онцлох
            </span>
          </div>
        )}
      ...
      <div className="flex flex-col gap-1.5">
        <h3 className="text-text-primary font-medium text-base sm:text-lg font-manrope leading-6 line-clamp-2">
          {article.title}
        </h3>
        {displayDate && (
          <span className="text-text-secondary font-normal text-sm font-manrope">
