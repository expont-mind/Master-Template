...
export default async function DashboardPage({
  ...
  const dateRangeFilter = fromParam && toParam ? { from: fromParam, to: toParam } : null;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-2xl sm:text-3xl font-bold tracking-tight">Хяналтын самбар</h2>
          <p className="text-muted-foreground">{monthLabel || getTodayDateMN()}</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <RecentOrders orders={stats.recentOrders} isFiltered={stats.isFiltered} />
