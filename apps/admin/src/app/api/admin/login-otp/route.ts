...
function verifyOtpToken(
  ...
  try {
    ...
    if (Date.now() > data.exp) return { valid: false, error: "Кодын хугацаа дууссан" };

    const codeHash = crypto.createHash("sha256").update(code).digest("hex");
    if (data.codeHash !== codeHash) return { valid: false, error: "Код буруу байна" };
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!twoFactorEnabled) {
      return NextResponse.json({ error: "2FA идэвхгүй байна" }, { status: 400 });
    }
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (!email || !code || !otpToken) {
      return NextResponse.json({ error: "email, code, otpToken шаардлагатай" }, { status: 400 });
    }

    // OTP шалгах
    const result = verifyOtpToken(otpToken, email, code);
    if (!result.valid) {
      return NextResponse.json({ error: result.error || "Код буруу байна" }, { status: 400 });
    }
    ...
    if (linkError || !linkData?.properties?.hashed_token) {
      ...
      return NextResponse.json({ error: "Session үүсгэхэд алдаа гарлаа" }, { status: 500 });
    }
