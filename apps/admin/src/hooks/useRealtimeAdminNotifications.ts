...
export function useUnreadAdminNotificationCount() {
  return useQuery({
    ...
    queryFn: async () => {
      const result = await adminApi.getAllPaginated<AdminNotification>("admin_notifications", {
...
export function useMarkAllAdminNotificationsRead() {
  ...
  return useMutation({
    mutationFn: async () => {
      const unreadNotifications = await adminApi.getAll<AdminNotification>("admin_notifications", {
        filters: { "is_read.is": "false" },
        select: "id",
      });
