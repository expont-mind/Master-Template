...
export function useUnreadNotificationCount() {
  return useQuery({
    ...
    queryFn: async () => {
      const result = await adminApi.getAllPaginated<Notification>("notifications", {
...
export function useMarkAllNotificationsRead() {
  ...
  return useMutation({
    mutationFn: async () => {
      ...
      const unreadNotifications = await adminApi.getAll<Notification>("notifications", {
        filters: { "is_read.is": "false" },
        select: "id",
      });
      ...
      // Mark each as read
      await Promise.all(
        unreadNotifications.map((n) => adminApi.update("notifications", n.id, { is_read: true })),
      );
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.notifications.all });
