...
export function useReviewEdit(id: string) {
  ...
  const saveMutation = useMutation({
    mutationFn: (data: Record<string, unknown>) => adminApi.update("reviews", id, data),
    onSuccess: () => {
    ...
    try {
      ...
      setError(
        translateServerError(
          ...
          "Сэтгэгдэл хадгалахад алдаа гарлаа.",
        ),
      );
    } finally {
      setIsSaving(false);
  ...
  const getProductImage = () => {
    ...
    const primary = review.products.product_images.find((img) => img.is_primary);
