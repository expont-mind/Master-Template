...
function aggregateByDay(items: { created_at: string }[], dateFrom: Date, dateTo: Date): DayValue[] {
...
export function useUserAnalytics() {
  // Current month range. Memoized so the Date instances stay stable across
  // renders (otherwise downstream useMemo deps change every render).
  ...
  const analytics = useMemo(() => {
    ...
    const newUsers = users.filter((u) => u.created_at >= fromStr && u.created_at <= toStr);
    ...
    const totalBalance = Array.from(balanceMap.values()).reduce((sum, b) => sum + b, 0);
    const avgPointBalance = balanceMap.size > 0 ? Math.round(totalBalance / balanceMap.size) : 0;
