...
export function useUserEdit(id: string) {
  ...
  const { data: user = null, isLoading: isLoadingUser } = useQuery({
    ...
    queryFn: () =>
      adminApi.getById<User>("users", id, {
        select:
          "id, email, first_name, last_name, primary_phone, avatar_url, status, created_at, updated_at, point_activated_at, addresses(id, city, district, sub_district, detail, is_default)",
      }),
  });
  ...
  const saveMutation = useMutation({
    mutationFn: (data: Record<string, unknown>) => adminApi.update("users", id, data),
    onSuccess: () => {
    ...
    try {
      ...
      setError(
        translateServerError(
          ...
          "Хэрэглэгчийн мэдээлэл хадгалахад алдаа гарлаа.",
        ),
      );
    } finally {
      setIsSaving(false);
