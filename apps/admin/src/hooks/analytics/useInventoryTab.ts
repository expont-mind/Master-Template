...
type StockBucket = { label: string; min: number; max: number; count: number };

function buildStockBuckets(products: ProductRow[]): StockBucket[] {
...
function summarizeSkus(products: ProductRow[]): {
  totalSkus: number;
  outOfStock: number;
  lowStock: number;
  ...
  return { totalSkus, outOfStock, lowStock, totalStockValue };
}

function buildStockByCategory(
  products: ProductRow[],
...
export function useInventoryTab(dr: AnalyticsDateRange, isActive: boolean) {
  ...
  const data = useMemo(() => {
    ...
    return {
      ...summarizeSkus(activeProducts),
      stockBuckets: buildStockBuckets(activeProducts),
      inventoryAlerts,
      stockByCategory: buildStockByCategory(activeProducts, allCategories),
    };
