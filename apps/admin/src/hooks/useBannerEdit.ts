...
export function useBannerEdit(id?: string) {
  ...
  useEffect(() => {
    if (banner) {
      // eslint-disable-next-line react-hooks/set-state-in-effect -- prefill form fields from saved banner after async fetch resolves
      setFormData({
  ...
  const saveMutation = useMutation({
    mutationFn: (data: Record<string, unknown>) =>
      isEditMode && id ? adminApi.update("banners", id, data) : adminApi.insert("banners", data),
    onSuccess: () => {
    ...
    try {
      ...
      setError(
        translateServerError(
          ...
          "Баннер хадгалахад алдаа гарлаа.",
        ),
      );
    } finally {
      setIsSaving(false);
