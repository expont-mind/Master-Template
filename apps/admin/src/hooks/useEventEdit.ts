...
export function useEventEdit(id: string) {
  ...
  useEffect(() => {
    if (event) {
      // eslint-disable-next-line react-hooks/set-state-in-effect -- prefill form fields from saved event after async fetch resolves
  ...
  const saveMutation = useMutation({
    mutationFn: (data: Record<string, unknown>) =>
      isNew ? adminApi.insert("events", data) : adminApi.update("events", id, data),
    onSuccess: () => {
