...
async function upsertSmsBalance(newAmount: number) {
  ...
  const existing = await adminApi.getAll<{ id: string; key: string }>("settings", {
...
export function SmsBalanceCard({ balance, spent, sentCount }: SmsBalanceCardProps) {
    ...
    <Card>
      <CardContent className="p-4 sm:pt-6 sm:px-6">
        <div className="flex items-center justify-between">
          <p className="text-xs sm:text-sm text-muted-foreground">SMS үлдэгдэл</p>
        ...
        <div className="text-[11px] sm:text-xs text-muted-foreground hidden sm:block space-y-0.5">
          <p>
            Цэнэглэсэн: ₮{balance.toLocaleString()} / Зарцуулсан: ₮{spent.toLocaleString()}
          </p>
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="sm:max-w-md">
            ...
            <div className="space-y-4">
              ...
              <Button onClick={handleFill} disabled={loading || !amount} className="w-full">
