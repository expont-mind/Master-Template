...
export function KpiStatCards({ stats }: KpiStatCardsProps) {
  const cards = [
    ...
    },
    {
      ...
      icon: ShoppingCart,
      iconBg:
        stats.pendingOrders > 0 ? "bg-yellow-100 text-yellow-700" : "bg-green-100 text-green-700",
    },
    {
      title: stats.isFiltered ? "Хэрэглэгч" : "Нийт хэрэглэгч",
      value: stats.totalUsers.toLocaleString(),
      description: stats.isFiltered ? "Энэ сард бүртгүүлсэн" : "Бүртгэлтэй хэрэглэгчид",
      icon: Users,
      ...
      {cards.map((card) => (
        <Card key={card.title}>
          <CardContent className="p-4 sm:pt-6 sm:px-6">
            ...
            <p className="mt-1 sm:mt-2 text-lg sm:text-2xl font-bold">{card.value}</p>
