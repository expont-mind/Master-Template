      ...
      <div className="flex items-center gap-4 flex-1 min-w-0">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <Link href={`/ads/${ad.id}`} className="font-medium hover:underline truncate">
              {ad.title}
            </Link>
            <Badge variant="outline">{AD_POSITION_LABELS[ad.position]}</Badge>
            {isExpired ? (
          ...
          <div className="flex items-center gap-4 mt-1 text-sm text-muted-foreground">
            ...
            {ad.start_date && (
              <span>
                {new Date(ad.start_date).toLocaleDateString("mn-MN", {
                  timeZone: "Asia/Ulaanbaatar",
                })}
                {ad.end_date &&
                  ` - ${new Date(ad.end_date).toLocaleDateString("mn-MN", { timeZone: "Asia/Ulaanbaatar" })}`}
              </span>
            )}
          </div>
        </div>
