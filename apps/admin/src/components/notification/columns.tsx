...
export function getColumns(options: {
  ...
  return [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div>
          <p className={`font-medium ${!row.original.is_read ? "text-primary" : ""}`}>
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return row.original.users ? (
          <span className="text-sm">{fullName || row.original.users.email}</span>
        ) : (
    ...
    },
    {
      ...
      header: "",
      size: 60,
      cell: ({ row }) => <ActionsCell onDelete={() => options.onDelete?.(row.original)} />,
    },
  ];
}
