  ...
  return (
    <div className="flex flex-col gap-[22px] pb-1.5">
      ...
      <div className="flex items-center gap-2">
        <Switch id="category-featured" checked={isFeatured} onCheckedChange={setIsFeatured} />
        <Label htmlFor="category-featured" className="text-sm font-medium cursor-pointer">
          Онцлох
        </Label>
      </div>
      <div className="flex items-start gap-2">
        <Switch id="category-status" checked={isActive} onCheckedChange={setIsActive} />
        <Label htmlFor="category-status" className="text-sm font-medium cursor-pointer">
...
export function CategoryDialog({
  ...
  useEffect(() => {
    if (open) {
      // eslint-disable-next-line react-hooks/set-state-in-effect -- reset dialog fields to current category values each time it opens
      setName(initialName);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange} modal={false}>
      ...
      >
        <DialogDescription className="sr-only">{TITLES[mode]}</DialogDescription>
        {/* Header */}
        <div className="flex items-center justify-between">
          <DialogTitle className="text-xl font-semibold leading-7">{TITLES[mode]}</DialogTitle>
          <button
            type="button"
        ...
        <div className="flex items-center justify-end gap-2">
          ...
          <Button size="sm" onClick={handleSave} disabled={isSaving} className="rounded">
