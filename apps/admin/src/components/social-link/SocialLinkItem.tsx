...
function SocialLinkEditForm({
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
      <Select value={editPlatform.toLowerCase()} onValueChange={onPlatformChange}>
...
function SocialLinkDisplay({ link, platformLabel }: { link: SocialLink; platformLabel: string }) {
  return (
    <>
...
export function SocialLinkItem({
  ...
  const platformData = getPlatformData(isEditing ? editPlatform : link.platform);
  const IconComponent = platformData?.icon ?? Share2;
  const iconColor = platformData?.color;
  const platformLabel = platformData?.label ?? link.platform;
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        {isEditing ? (
          ...
          <SocialLinkDisplay link={link} platformLabel={platformLabel} />
        )}
      </div>
