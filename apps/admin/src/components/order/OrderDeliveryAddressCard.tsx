  ...
  return (
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="text-sm font-semibold">Хүргэлтийн хаяг</CardTitle>
      </CardHeader>
      <CardContent className="space-y-0">
        <div className="py-2.5 border-b border-border">
          <p className="text-sm font-medium text-foreground">Хүргэлтийн бүс: {deliveryZone}</p>
        </div>
        {address && (
          <div className="py-2.5">
            ...
            {(recipientName || phoneNumbers.length > 0) && (
              <div className="mt-1 space-y-0.5">
                {recipientName && <p className="text-sm text-slate-500">{recipientName}</p>}
                {phoneNumbers.length > 0 && (
                  <div className="flex items-center gap-0.5 text-sm text-slate-500">
                    {phoneNumbers.map((phone, index) => (
                      <span key={phone}>
                        {phone}
                        {index < phoneNumbers.length - 1 && <span className="mx-0.5">·</span>}
                      </span>
                    ))}
                  </div>
                )}
