...
Deno.serve(async (req) => {
  ...
  if (error) {
    log.error("[check-pending] Query error:", error.message);
    ...
    try {
      if (invoice.provider === "qpay") {
        ...
        if (isPaid && paidAmount > 0) {
          ...
          if (rpcError) {
            log.error(`[check-pending] RPC error for invoice ${invoice.id}:`, rpcError.message);
          } else {
            const rpcResult = rpcData as { success: boolean; error?: string } | null;
            if (rpcResult && !rpcResult.success) {
              log.error(`[check-pending] RPC failure for invoice ${invoice.id}:`, rpcResult.error);
            }
          }
      ...
      log.error(`[check-pending] Error processing invoice ${invoice.id}:`, err);
    }
  }
