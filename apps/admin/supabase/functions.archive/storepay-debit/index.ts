...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { action, amount, description, phoneNumber, orderNumber, userId, orderId } =
      (await req.json()) as {
        action: string;
        amount?: number;
        description?: string;
    ...
    if (insertError) {
      log.error("[storepay-debit] DB insert error:", insertError.message);
    ...
    log.error("[storepay-debit] Error:", err);
    return jsonResponse(
      {
        success: false,
