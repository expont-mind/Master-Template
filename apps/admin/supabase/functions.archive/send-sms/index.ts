...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (result.success) {
      log.info(`[send-sms] ${provider} ok phone=${phone} messageId=${result.messageId ?? "-"}`);
    } else {
      log.error(
        `[send-sms] ${provider} fail phone=${phone} error=${result.error} raw=${result.rawResponse ?? "-"}`,
      );
    }
    ...
    supabase
      ...
      .insert({
        ...
        provider,
        provider_message_id: result.messageId ?? result.rawResponse?.slice(0, 255) ?? null,
