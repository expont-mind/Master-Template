...
function validateAddress(addr: DeliveryAddress): { valid: true } | { valid: false; error: string } {
  ...
  if (!addr.sub_district) return { valid: false, error: "Хороо/Баг сонгоно уу" };
  if (!addr.detail?.trim()) return { valid: false, error: "Дэлгэрэнгүй хаяг оруулна уу" };
  return { valid: true };
}
...
async function validateOrderItems(
  ...
  const variantIds = [
    ...new Set(items.map((i) => i.variantId).filter((id): id is string => id != null)),
  ];
  ...
  const productMap = new Map<
    string,
    { id: string; name: string; is_active: boolean; stock_quantity: number }
  >(
    products.map((p: { id: string; name: string; is_active: boolean; stock_quantity: number }) => [
      p.id,
      p,
    ]),
  );
  const variantMap = new Map<
    string,
    { id: string; product_id: string; name: string | null; stock_quantity: number; status: string }
  >(
...
async function createQPayInvoice(
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      log.info(`[createQPayInvoice] attempt ${attempt}, url=${url}`);
      ...
      log.error(`[createQPayInvoice] attempt ${attempt} failed:`, lastError.message);
      // Only retry on network errors (fetch failed), not on API errors
      if (
        !lastError.message.includes("fetch failed") &&
        !lastError.message.includes("error sending request")
      ) {
        throw lastError;
      }
      if (attempt < 3) {
...
async function handleFreeOrder(
  ...
  const deliveryFee = await calculateDeliveryFee(admin, body.address.city, itemsTotal);
  ...
  if (orderError || !order) {
    log.error("[handleFreeOrder] Order creation error:", orderError);
    return new Response(
  ...
  if (itemsError) {
    log.error("[handleFreeOrder] Order items error:", itemsError);
  ...
  try {
    ...
    log.error("[handleFreeOrder] Stock decrement error:", err);
  }
    ...
    try {
      ...
      if (coupon) {
        if (coupon.usage_limit && (coupon.usage_count ?? 0) >= coupon.usage_limit) {
          log.warn("[handleFreeOrder] Coupon global usage limit reached");
          canRecord = false;
        } else {
          ...
          if ((count ?? 0) >= perUserLimit) {
            log.warn("[handleFreeOrder] Coupon per-user usage limit reached");
            canRecord = false;
          }
        }
      }
      ...
      log.error("[handleFreeOrder] Coupon usage error:", err);
    }
  }
  ...
  if ((body.pointsUsed ?? 0) > 0) {
    try {
      ...
      log.error("[handleFreeOrder] Point usage error:", err);
    }
  }
  ...
  try {
    ...
    log.error("[handleFreeOrder] Award points error:", err);
  }
...
async function handleExistingOrderRetry(
  ...
  if (orderError || !order) {
    return new Response(JSON.stringify({ success: false, error: "Захиалга олдсонгүй" }), {
      status: 404,
  ...
  if (order.user_id !== userId) {
    return new Response(JSON.stringify({ success: false, error: "Зөвшөөрөлгүй" }), {
      status: 403,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
  ...
  if (!itemsCheck.valid) {
    return new Response(JSON.stringify({ success: false, error: itemsCheck.error }), {
      status: 400,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
  ...
  if (insertError) {
    log.error("[handleExistingOrderRetry] Invoice insert error:", insertError);
    return new Response(
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    log.info("[create-checkout-invoice] === REQUEST START ===");
    log.info("[create-checkout-invoice] method:", req.method);
    const headerEntries: string[] = [];
    req.headers.forEach((value, key) => {
      // Mask sensitive values but show presence
      if (key.toLowerCase() === "authorization") {
        headerEntries.push(
          `${key}: ${value.substring(0, 15)}...${value.substring(value.length - 10)}`,
        );
    ...
    log.info("[create-checkout-invoice] headers:", headerEntries.join(" | "));

    // Authenticate user via JWT
    const authHeader = req.headers.get("Authorization");
    log.info(
      "[create-checkout-invoice] authHeader present:",
      !!authHeader,
      "length:",
      authHeader?.length,
    );
    if (!authHeader) {
      log.info("[create-checkout-invoice] NO Authorization header — 401");
      return new Response(JSON.stringify({ success: false, error: "Нэвтэрч орно уу" }), {
        status: 401,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    log.info("[create-checkout-invoice] JWT length:", jwt.length);
    const authResult = await admin.auth.getUser(jwt).catch((err: unknown) => {
      log.error("[create-checkout-invoice] getUser THREW:", err);
      return null;
    });
    const user = authResult?.data?.user;
    log.info("[create-checkout-invoice] user found:", !!user, "userId:", user?.id);
    if (!user) {
      log.info("[create-checkout-invoice] User null — 401");
      return new Response(JSON.stringify({ success: false, error: "Нэвтэрч орно уу" }), {
        status: 401,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    ...
    if (!body.items || body.items.length === 0) {
      return new Response(JSON.stringify({ success: false, error: "Сагс хоосон байна" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!addrCheck.valid) {
      return new Response(JSON.stringify({ success: false, error: addrCheck.error }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (orderError || !order) {
      log.error("[create-checkout-invoice] Order error:", orderError);
      return new Response(
        JSON.stringify({
          success: false,
    ...
    if (itemsError) {
      log.error("[create-checkout-invoice] Items error:", itemsError);
      await admin.from("orders").delete().eq("id", order.id);
      return new Response(
        JSON.stringify({
          success: false,
    ...
    if (insertError) {
      log.error("[create-checkout-invoice] Invoice insert error:", insertError);
      await admin.from("orders").delete().eq("id", order.id);
      return new Response(
        JSON.stringify({
          success: false,
    ...
    log.error("[create-checkout-invoice] Error:", err);
    return new Response(
      JSON.stringify({
        success: false,
