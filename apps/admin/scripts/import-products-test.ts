...
async function main() {
  ...
  try {
    ...
    console.log(
      `Testing with products ${TEST_OFFSET + 1}-${TEST_OFFSET + TEST_LIMIT} (offset: ${TEST_OFFSET})\n`,
    );
    ...
    if (successResults.length === newProducts.length) {
      ...
      console.log(
        "  - Images without _t prefix → Rich description images (p_rich_description.images)",
      );
      console.log("\nTo run the full import (156 products), use:");
      console.log("  npx tsx scripts/import-products.ts\n");
    } else {
      console.log(
        "⚠️  Some products failed. Please review errors above before running full import.\n",
      );
    }
