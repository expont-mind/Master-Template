...
async function main() {
  ...
  sampleProducts.forEach((product, idx) => {
    ...
    if (mainImages.length > 0) {
      console.log(
        `   Main: ${mainImages.slice(0, 2).join(", ")}${mainImages.length > 2 ? ` ... +${mainImages.length - 2} more` : ""}`,
      );
    }
    if (descImages.length > 0) {
      console.log(
        `   Desc: ${descImages.slice(0, 2).join(", ")}${descImages.length > 2 ? ` ... +${descImages.length - 2} more` : ""}`,
      );
    }
    console.log("");
  });
  ...
  console.log(
    `Total main images (_t): ${totalMainImages} (avg: ${(totalMainImages / totalProducts).toFixed(1)} per product)`,
  );
  console.log(
    `Total desc images (no _t): ${totalDescImages} (avg: ${(totalDescImages / totalProducts).toFixed(1)} per product)`,
  );
  console.log("─────────────────────────────────────────────────────────\n");
